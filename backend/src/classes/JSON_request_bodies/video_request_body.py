...
class MainVideoSettings(VideoSettings):
    num_of_images: int = Field(..., gt=0, example=10, description="Number of images to take") # 1 is default
    gain: int = Field(1, gt=0, example=1)
    
class TestVideoSettings(VideoSettings):
    min_gain: float = Field(..., ge=1)
    max_gain: float = Field(..., le=10)
    gain_step: float = Field(...) # increment between gain values tested