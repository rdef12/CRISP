...
def fitBP(z, D, unc_D=None, method='bortfeld', rel_resolution=0.01):
    ...
    if method == 'bortfeld':
        ...
        #################### I have added uncertainties in #######################################
        if unc_D is not None:
            # print("Y_errs being used when Bortfeld fitting")
            p, c = curve_fit(
                bortfeld, z, D,
                p0 = [A, R0, sigma, p, k],
                bounds=( # limits
                ...
                sigma=unc_D, absolute_sigma=True
            )
        else:
            p, c = curve_fit(
                bortfeld, z, D,
...
def fitBP_odr(z, D, z_unc, D_unc, method='bortfeld', rel_resolution=0.01, use_non_ODR_estimates: bool=False):
   ...
   if method == 'bortfeld':
       ...
       if use_non_ODR_estimates:
           print("TEST")
           bortfeld_fit = fitBP(z, D, D_unc)
           original_fit_parameters = bortfeld_fit['bortfeld_fit_p']
           myodr = odr.ODR(mydata, function_model, beta0=original_fit_parameters, maxit=1000)
       else:
           myodr = odr.ODR(mydata, function_model, beta0=[A, R0, sigma, p, k], maxit=1000)
...
def fit_bortfeld_odr(z_bins, on_axis_energies, z_uncertainties, on_axis_energies_uncertainties, use_non_ODR_estimates: bool=False):

   bortfeld_fit = fitBP_odr(z_bins, on_axis_energies, z_uncertainties, on_axis_energies_uncertainties, use_non_ODR_estimates=use_non_ODR_estimates)
