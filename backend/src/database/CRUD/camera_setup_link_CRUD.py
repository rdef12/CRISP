...
def get_initial_horizontal_region_of_interest(camera_id:int, setup_id:int) -> tuple[int, int]:
    with Session(engine) as session:
        statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
        result = session.exec(statement).one()
        if result:
            return result.initial_horizontal_region_of_interest
        else:
            raise ValueError(f"Initial horizontal ROI not found for camera with id {camera_id} and setup with id {setup_id}.")

def get_initial_vertical_region_of_interest(camera_id:int, setup_id:int) -> tuple[int, int]:
    with Session(engine) as session:
        statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
        result = session.exec(statement).one()
        if result:
            return result.initial_vertical_region_of_interest
        else:
            raise ValueError(f"Initial vertical ROI not found for camera with id {camera_id} and setup with id {setup_id}.")

def get_intial_region_of_interest_photo(camera_id:int, setup_id:int) -> bytes:
    with Session(engine) as session:
        statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
        result = session.exec(statement).one()
        if result:
            return result.initial_region_of_interest_photo
        else:
            raise ValueError(f"Far face calibration photo not found for camera with id {camera_id} and setup with id {setup_id}.")
...
def update_initial_region_of_interest_camera_settings_link(camera_id:int, setup_id:int, camera_settings_id:int):
    try:
        with Session(engine) as session:
            statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
            camera_setup_link = session.exec(statement).one()
            camera_setup_link.initial_region_of_interest_photo_camera_settings_link = camera_settings_id
            session.commit()
            return {"message": f"Camera settings link updated for camera with id {camera_id} and setup with id {setup_id}."}
    except NoResultFound:
        raise ValueError(f"No camera setup link found for camera_id={camera_id} and setup_id={setup_id}.")
...
def update_initial_region_of_interest_photo(camera_id:int, setup_id:int, initial_region_of_interest_photo_bytes: bytes):
    try:
        with Session(engine) as session:
            statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
            result = session.exec(statement).one()
            result.initial_region_of_interest_photo = initial_region_of_interest_photo_bytes
            session.commit()
            return {"message": f"Far face calibration photo updated for camera with id {camera_id} and setup with id {setup_id}."}
    except NoResultFound:
        raise ValueError(f"No camera setup link found for camera_id={camera_id} and setup_id={setup_id}.")
...
def update_initial_horizontal_region_of_interest(camera_id:int, setup_id:int, initial_horizontal_region_of_interest: tuple[int, int]):
    try:
        with Session(engine) as session:
            statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
            result = session.exec(statement).one()
            result.initial_horizontal_region_of_interest = initial_horizontal_region_of_interest
            session.commit()
            return {"message": f"Initial horizontal ROI updated for camera with id {camera_id} and setup with id {setup_id}."}
    except NoResultFound:
        raise ValueError(f"No camera setup link found for camera_id={camera_id} and setup_id={setup_id}.")
    except Exception as e:
        raise RuntimeError(f"An error occurred: {str(e)}")
...
def update_initial_vertical_region_of_interest(camera_id:int, setup_id:int, initial_vertical_region_of_interest: tuple[int, int]):
    try:
        with Session(engine) as session:
            statement = select(CameraSetupLink).where(CameraSetupLink.camera_id == camera_id).where(CameraSetupLink.setup_id == setup_id)
            result = session.exec(statement).one()
            result.initial_vertical_region_of_interest = initial_vertical_region_of_interest
            session.commit()
            return {"message": f"Initial vertical ROI updated for camera with id {camera_id} and setup with id {setup_id}."}
    except NoResultFound:
        raise ValueError(f"No camera setup link found for camera_id={camera_id} and setup_id={setup_id}.")
    except Exception as e:
        raise RuntimeError(f"An error occurred: {str(e)}")
